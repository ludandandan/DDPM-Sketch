...
class SinusoidalPositionEmbedding(nn.Module):
    def __init__(self, total_time_step=1000, time_emb_dims=128, time_emb_dim_exp=512):
        super().__init__()
        ...
        ts = torch.arange(total_time_step, dtype=torch.float32)
        ...
        self.time_blocks = nn.Sequential(
            ...
            nn.Linear(in_features=time_emb_dims, out_features=time_emb_dim_exp),
            nn.SiLU(), # x*sigmoid(x)
            nn.Linear(in_features=time_emb_dim_exp, out_features=time_emb_dim_exp)
        )
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels, dropout_rate=0.1, time_emb_dims=512, apply_attention=False):
        ...
        self.conv_2 = nn.Conv2d(in_channels=self.in_channels, out_channels=self.out_channels, kernel_size=3, stride=1, padding="same")
...
class UNet(nn.Module):
    def __init__(
        ...
        self.time_embeddings = SinusoidalPositionEmbedding(time_emb_dims=base_channels, time_emb_dim_exp=time_emb_dims_exp)

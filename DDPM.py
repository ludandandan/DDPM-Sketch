...
class Diffusion_setting:
    def __init__(self, num_diffusion_timesteps=1000, img_shape=(3,64,64), device="cpu"):
        ...
        self.initialize()
    def initialize(self):
        # calculate all the setting for every timesteps, store in a tensor matrix
        # BETAs and ALPhAs required at different places in the Algorithm
        self.betas = self.get_betas() # a set of beta(i)s
        self.alphas = 1 - self.betas # a set of alpha(i)s
...
def Denoising_onestep(model, DS:Diffusion_setting, X_t:torch.Tensor, timestep:torch.Tensor, start_at_T=False):
    ...
    var = get(DS.sqrt_beta_s, timestep) # todo here, the authers take the sqrt(beta_t) from diffusing process
    # instead of mathmetical results of [1-alpha_cumulative(t-1)]*beta_t/[1-alpha_cumulative(t)]
